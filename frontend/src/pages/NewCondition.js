...
export default function NewCondition() {
  ...
  useEffect(() => {
    api.get('/indicators/metadata').then(res => {
      setMetadata(res.data);
      const first = Object.keys(res.data.indicators)[0];
      setIndicatorType(first);
      setOperator(res.data.operators[0].value);
  ...
  useEffect(() => {
    if (metadata && indicatorType) {
      const dp = {};
      const cfg = metadata.indicators[indicatorType];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post(`/strategies/${id}/conditions/`, { indicator_type: indicatorType, params, lookback_days: lookbackDays, operator, threshold });
      navigate(`/strategies/${id}/conditions`);
    } catch (err) {
      setError(err.response?.data?.detail || err.toString());
    }
  };

  if (!metadata) return <div style={{ textAlign: 'center', padding: '6rem 0', color: '#94a3b8', animation: 'pulse 1.5s infinite' }}>Loading...</div>;
  const ci = metadata.indicators[indicatorType];

  return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#f1f5f9', marginBottom: '1.25rem' }}>New Condition — Strategy #{id}</h2>
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '14px', padding: '1.5rem' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <button type="submit" style={{ flex: 1, padding: '0.65rem', background: '#6366f1', color: '#ffffff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}>Create Condition</button>
            <Link to={`/strategies/${id}/conditions`} style={{ textDecoration: 'none', textAlign: 'center', padding: '0.6rem 1rem', background: '#334155', color: '#94a3b8', borderRadius: '8px', fontWeight: 500, fontSize: '0.85rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Cancel</Link>
          </div>
        </form>
      </div>
    </div>
